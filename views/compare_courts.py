...
def _court_kpi_col(col, court_name: str, cdf: pd.DataFrame, total_all: int, card_key: str, cat_col: str = "Category_Group", cat_label: str = "Category Group"):
...
def render():
    ...
    ka, kb = st.columns(2)
    if len(df_a):
        _court_kpi_col(ka, court_a, df_a, total_sel, "card_cc_h2h_a", cat_col=cat_col, cat_label=cat_label)
    else:
        with ka:
            st.info(f"No data for {court_a} in the current filter period.")
    if len(df_b):
        _court_kpi_col(kb, court_b, df_b, total_sel, "card_cc_h2h_b", cat_col=cat_col, cat_label=cat_label)
    else:
        with kb:
    ...
    if len(df_a) and len(df_b):
        ...
        with h2h_c2:
            with st.container(key="card_cc_h2h_trend"):
                ...
                df_h2h  = df[df["Court"].isin([court_a, court_b])].dropna(subset=["Year_Month"])
        ...
        with st.container(key="card_cc_h2h_delta"):
            ...
            if vol_a > 0 and vol_b > 0:
                ...
                insight_pill(
                    ...
                    f"<b>{bigger_court} High Court</b> has <b>{pct_diff:.1f}% more</b> listed cases than "
    ...
    total_cases = len(df)
    active_courts = df["Court"].nunique() if total_cases else 0
    top_court = df["Court"].value_counts().idxmax() if total_cases else "N/A"
    top_court_cases = df["Court"].value_counts().max() if total_cases else 0
